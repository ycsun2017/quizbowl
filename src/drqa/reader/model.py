...
class DocReader(object):
    ...
    def update(self, ex):
        ...
        # Transfer to GPU
        if self.use_cuda:
            inputs = [e if e is None else Variable(e.cuda(async_=True))
                      for e in ex[:5]]
            target_s = Variable(ex[5].cuda(async_=True))
            target_e = Variable(ex[6].cuda(async_=True))
        else:
    ...
    def predict(self, ex, candidates=None, top_n=1, async_pool=None):
        ...
        if self.use_cuda:
            inputs = [e if e is None else
                      Variable(e.cuda(async_=True), volatile=True)
                      for e in ex[:5]]
        else:
